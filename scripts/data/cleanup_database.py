...
def find_duplicates(client: OdooClient) -> dict[str, list[dict]]:
    ...
    if duplicates:
        print(
            f"   Found {len(duplicates)} SKUs with duplicates ({sum(len(p) for p in duplicates.values())} total products)"
        )
    else:
...
def delete_duplicates(
    client: OdooClient, duplicates: dict[str, list[dict]], dry_run: bool = True
) -> int:
...
def main():
    ...
    parser.add_argument(
        "--delete-empty-sets", action="store_true", help="Delete empty category sets"
    )
    parser.add_argument(
        "--stats-only", action="store_true", help="Only show statistics, don't look for issues"
    )
    ...
    if args.stats_only:
        ...
        for s in stats:
            img_status = f"{s['with_images']}/{s['total_products']}"
            print(
                f"{s['name'][:35]:<35} {s['total_products']:>6} {img_status:>7} {s['in_stock']:>6}"
            )
        print("-" * 60)
